...
class EnsembleNNBandit(Bandit):
    def __init__(self, n_arms, n_features, R=1, M=10, epochs=1, l2=.1): #, data, y):
        super().__init__(n_arms)
        self.n_features = n_features
        self.R = R
        self.vsq = R**2 
        ...
        self.M = M
        self.device = 'cpu'
        self.data = [ torch.Tensor().to(self.device) for _ in range(self.n_arms) ]
        self.y = [ [ torch.Tensor().to(self.device) for _ in range(self.M) ] 
                        for _ in range(self.n_arms) ]
      
        self.models = [ [ sklearn.neural_network.MLPRegressor(hidden_layer_sizes=(),
            max_iter=epochs, solver='sgd', tol=1e-2, activation='identity', alpha=1).fit(
                np.zeros((1, n_features)),np.zeros((1, ))) 
            for m in range(self.M) ] for a in range(self.n_arms) ]
        self.n_epochs = epochs
        self.batch_size = 32  

    @timing
    def _train(self, print_info=True):    
        for a in range(self.n_arms):
            if len(self.data[a]) == 0: 
                continue
            for m, mdl in enumerate(self.models[a]):
                if print_info: print(a, m, end=',')
                for _ in range(self.n_epochs):
                    mdl.fit(self.data[a], self.y[a][m])
               
    @timing  
    def _eval(self, X: np.ndarray):
        x = torch.Tensor(X).to(device=self.device)
        ret = [ [ [] for _ in range(self.M) ] for _ in range(self.n_arms) ]
        #with torch.no_grad():
        for a in range(self.n_arms):
            if len(self.data[a]) == 0: 
                continue
            for m, mdl in enumerate(self.models[a]):        
                #mdl.eval()
                ret[a][m].append(mdl.predict(x)) #.cpu().numpy())
        return ret
    ...
    @timing
    def pick(self, X: np.ndarray):
        x = X[None] #torch.Tensor(X).to(device=self.device)
        ms = np.random.randint(0, self.M, size=self.n_arms)
        with torch.no_grad():
            # p = np.array([ self.models[i][ms[i]](x).cpu().numpy() for i in range(self.n_arms) ])
            p = np.array([ self.models[i][ms[i]].predict(x) for i in range(self.n_arms) ])
        return round(np.argmax(p))
 
    @timing
    def update(self, X: np.ndarray, a, r):
        super().update(X, a, r)
        new_sample = torch.Tensor(X).to(self.device).reshape(-1, self.n_features)
        self.data[a] = torch.cat((self.data[a], new_sample)) 
        rnorms = np.random.randn(self.M)*self.vsq
        new_ys = [ torch.Tensor([r + z]).to(self.device) for z in rnorms ]
        for i, ny in enumerate(new_ys):
            self.y[a][i] = torch.cat((self.y[a][i], ny)) 
        self._train(print_info=False)
